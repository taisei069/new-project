...
class GameScreen31 extends StatefulWidget {
  const GameScreen31({super.key});

  @override
  State<GameScreen31> createState() => _GameScreenState31();
}

class _GameScreenState31 extends State<GameScreen31>
  ...
  var questions = QuestionData_3.all;//コピペ注意
  ...
  void _navigateToResultScreen(bool isCorrect) {
    ...
    Future.delayed(const Duration(milliseconds: 50), () {
      if (mounted) {
        context.go('/creative_31_1', extra: {'isCorrect': isCorrect, 'id': questions[indexCounter_creative.indexCount-1]['id']});
      }
    });
  }
...
Future<void> _savePart1Flag() async {
  try { // ▼▼▼ tryを追加 ▼▼▼
    ...
    await FirebaseFirestore.instance.runTransaction((tx) async {
      ...
      if (current >= 3) return;
      tx.set(docRef, {'part_3': 3}, SetOptions(merge: true)); // ← 保存するフラグ名を確認
    });
...
class TargetImageWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // ★★★ ここからが修正箇所 ★★★
    return Container(
      width: _GameScreenState31.targetSize,
      height: _GameScreenState31.targetSize,
