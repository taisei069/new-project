...
class St_pro_easy_quake6 extends StatelessWidget {
  const St_pro_easy_quake6({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    final List<Quiz_3> quizList = extra['quizList'];
...
final uid = FirebaseAuth.instance.currentUser!.uid;
  ...
  if (correctCount == totalCount) {
    await _savePart1Flag(); // Firestore に進捗記録
  }

  context.go('/diffculty_quake');
}

// Firestoreに "part_1": 3 を記録（進捗記録）
Future<void> _savePart1Flag() async {
  ...
  await FirebaseFirestore.instance.runTransaction((tx) async {
    ...
    tx.set(docRef, {'part_1': 3}, SetOptions(merge: true));
  });
}
